...
namespace UriTemplateTests
{
    public class UsageTests
    {
        ...
        {
            var template = new UriTemplate("https://foo.com/?format={+format}");
        ...
        {
            var template = new UriTemplate("https://foo.com/{?format}");
            template.SetParameter("format", "application/vnd.foo+xml");
        ...
        {
            var template = new UriTemplate("https://api.github.com/search/code?q={query}{&page,per_page,sort,order}");
        ...
        {
            var template = new UriTemplate("http://example.org/foo{?bar,baz}", true);
        ...
        public void ShouldHandleEncodingAParametersThatIsAUriWithAUriAsAParameter()
        {
            ...
            var uriString = template.Resolve();
            Assert.Equal(
                "http://example.org/go?uri=http%3A%2F%2Fexample.org%2F%3Furi%3Dhttp%253A%252F%252Fexample.org%252F",
                uriString);
        }
        ...
        {
            var template = new UriTemplate("/lookup{?Stra%C3%9Fe}");
        ...
        {
            var template = new UriTemplate("{?assoc_special_chars*}");
